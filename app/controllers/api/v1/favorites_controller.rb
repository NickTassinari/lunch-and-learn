    ...
    class FavoritesController < ApplicationController
      ...
      def create 
        ...
        if new_favorite.save && user 
          ...
          render json: { success: "Favorite successfully added" }, status: 201
        elsif !user 
      ...
      def destroy 
        user = User.find_by(api_key: params[:api_key])
        select_favorite = Favorite.find_favorite(favorite_params)
        if !user 
          render json: { message: "Invalid api_key" }, status: 401
        elsif !select_favorite || !user.favorites.include?(select_favorite)
          render json: { message: "Not Found" }, status: 404
        else   
          UsersFavorite.delete(select_favorite.id)
